...
@Component({
  selector: 'app-a-z-brands',
  templateUrl: './a-z-brands.component.html',
  styleUrls: ['./a-z-brands.component.scss']
})
export class AZBrandsComponent implements OnInit {
  ...
  private getBrands(): void {
    this.subscription = this.serviceProduct.getMyBrandsPag(this.pageCurrent).subscribe((data) => {
      this.brands = data.filter(product => product.type == 'regular' && product.gender == 'male');
    },
      err => {
        console.log(err);
      })
  ...
  getAllBrands(): void {
    this.serviceProduct.getMyProduct().subscribe(data => {
      let prodBrandsArr = data.filter(style => {
        return style.gender == 'male' && style.type == 'regular';
      });
      const brands = [];
    ...
    if (this.pagcol) {
      ...
      this.serviceProduct.trynewfilter(this.pagcol, this.pagbrnd, this.pagstyle, this.pagprc).subscribe(data => {
        this.brands = data.filter(prod => {
          return prod.gender === 'male' && prod.type == 'regular';
        })
      })
    }
    else {
