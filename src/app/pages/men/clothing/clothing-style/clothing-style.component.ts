...
@Component({
  selector: 'app-clothing-style',
  templateUrl: './clothing-style.component.html',
  styleUrls: ['./clothing-style.component.scss']
})
export class ClothingStyleComponent implements OnInit {
  ...
  private getData(currentStyle?): void {
    ...
    this.subscription = this.serviceProduct.getMyProductStylePag(this.page, currentStyle).subscribe((data) => {
      this.productStyle = data.filter(product => product.type == 'regular' && product.gender == 'male' && product.category == 'clothing' && product.style == `${currentStyle}`);
      console.log(this.productStyle);
    },
      err => {
        console.log(err);
      })
  ...
  filterColors(event): void {
    ...
    if (this.pagcol) {
      ...
      this.serviceProduct.trynewfilter(this.pagcol, this.pagbrnd, this.pagstyle, this.pagprc).subscribe(data => {
        this.productStyle = data.filter(prod => {
