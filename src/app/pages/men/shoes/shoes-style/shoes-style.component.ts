...
@Component({
  selector: 'app-shoes-style',
  templateUrl: './shoes-style.component.html',
  styleUrls: ['./shoes-style.component.scss']
})
export class ShoesStyleComponent implements OnInit {
  ...
  private getData(currentStyle?): void {
    ...
    this.subscription = this.serviceProduct.getMyProductStylePag(this.currentPage, currentStyle).subscribe((data) => {
      this.shoesStyle = data.filter(product => product.type == 'regular' && product.gender == 'male' && product.category == 'shoes' && product.style == `${currentStyle}`);
      console.log(this.shoesStyle);
    },
      err => {
        console.log(err);
      })
