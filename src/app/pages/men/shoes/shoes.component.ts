...
export class ShoesComponent implements OnInit {
  ...
  private getShoes(): void {
   this.subscription = this.serviceProduct.getMyShoesPag(this.currentPage).subscribe((data) => {
      this.shoes = data.filter(product => product.type == 'regular' && product.gender == 'male' && product.category == 'shoes');
    },
      err => {
        console.log(err);
      })
  ...
  getAllColors(): void {
    this.serviceProduct.getMyProduct().subscribe(data => {
      let prodColorsArr = data.filter(style => {
        return style.gender == 'male'&&style.category == 'shoes';
      });
      const colors = [];
    ...
    if (this.pagcol) {
      ...
      this.serviceProduct.trynewfilter(this.pagcol, this.pagbrnd, this.pagstyle, this.pagprc).subscribe(data => {
        this.shoes = data.filter(prod => {
          return prod.gender === 'male'&&prod.category ==='shoes';
        })
      })
    }
    else {
