...
@Component({
  selector: 'app-women-a-z-brands-style',
  templateUrl: './women-a-z-brands-style.component.html',
  styleUrls: ['./women-a-z-brands-style.component.scss']
})
export class WomenAZBrandsStyleComponent implements OnInit {
  ...
  private getData(currentStyle?): void {
    ...
    this.subscription = this.serviceProduct.getWomenBrandsStylePag(this.currentPage,currentStyle).subscribe((data) => {
      this.womenBrands = data.filter(product => product.type == 'regular' && product.gender == 'female'&& product.brand == `${currentStyle}`);
    },
      err => {
        console.log(err);
      })
  ...
  getAllStyles(currentStyle?): void {
    this.serviceProduct.getMyProduct().subscribe(data => {
      let newStylesArr = data.filter(style => {
        return style.gender == 'female'&&style.type ==='regular'&&style.brand == `${currentStyle}`;
      });
      const styles = [];
   ...
   filterColors(event): void {
    ...
    if (this.pagcol) {
      ...
      this.serviceProduct.trynewfilter(this.pagcol, this.pagbrnd, this.pagstyle, this.pagprc).subscribe(data => {
        this.womenBrands = data.filter(prod => {
          return prod.gender === 'female'&&prod.type == 'regular'&&prod.brand == this.routepage;
        })
      })
    }
    else {
