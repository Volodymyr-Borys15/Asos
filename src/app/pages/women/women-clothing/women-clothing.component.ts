...
export class WomenClothingComponent implements OnInit {
  ...
  private getClothing(): void {
    ...
    this.subscription = this.productService.getWomenClothingPagin(this.page).subscribe(data => {
      this.clothing = data;
      const colors = [];
    },
      err => {
  ...
  getAllStyles(): void {
    this.productService.getMyProduct().subscribe(data => {
      let clothingStylesArr = data.filter(style => {
        return style.gender == 'female'&&style.category == 'clothing';
      });
      const styles = [];
    ...
    if (this.pagstyle) {
      ...
      this.productService.trynewfilter(this.pagcol, this.pagbrnd, this.pagstyle, this.pagprc).subscribe(data => {
        this.clothing = data.filter(prod => {
          return prod.gender === 'female'&&prod.category ==='clothing';
        })
      })
    }
    else {
